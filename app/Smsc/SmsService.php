...
// $url = 'https://smsc.kz/sys/send.php?login=%s&psw=%s&phones=%d&mes=%s&sender=Spsahev Nurken&charset=utf-8';
// $text = sprintf($url, 's.nurken92@gmail.com','lenger92','87474646208','Hello world sms');
// $response = Http::post($text);
// dump($response->body());

class SmsService 
{
	const SMSC_LOGIN = "s.nurken92@gmail.com";			// логин клиента
	const SMSC_PASSWORD = "lenger92";	// пароль
	const SMSC_POST =  0;					// использовать метод POST
	const SMSC_HTTPS =  0;				// использовать HTTPS протокол	
	const SMSC_CHARSET = "UTF-8";	// кодировка сообщения: utf-8, koi8-r или windows-1251
	const SMSC_DEBUG = 0;				// флаг отладки
	const SMTP_FROM = "s.nurken92@gmail.com";     // e-mail адрес отправителя
	...
	{
		return mail("send@send.smsc.kz", "", SmsService::SMSC_LOGIN.":".SmsService::SMSC_PASSWORD.":$id:$time:$translit,$format,$sender:$phones:$message", "From: ".SmsService::SMTP_FROM."\nContent-Type: text/plain; charset=".SmsService::SMSC_CHARSET."\n");
	}
	...
	{
		$url = $_url = (SmsService::SMSC_HTTPS ? "https" : "http")."://smsc.kz/sys/$cmd.php?login=".urlencode(SmsService::SMSC_LOGIN)."&psw=".urlencode(SmsService::SMSC_PASSWORD)."&fmt=1&charset=".SmsService::SMSC_CHARSET."&".$arg;
	...
	{
		$ret = "";
		$post = SmsService::SMSC_POST || strlen($url) > 2000 || $files;
		...
		elseif ($files) {
			...
			if (!SmsService::SMSC_HTTPS && function_exists("fsockopen"))
			{
				$m = parse_url($url);
